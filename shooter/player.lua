...
player.behaviour[player.states.walking] = function (self, dt)
    ...
    if self.current_velocity[1] ~= 0 or self.current_velocity[2] ~= 0 then
        self.current_direction[1] = math.sign(self.current_velocity[1])
        self.current_direction[2] = math.sign(self.current_velocity[2])
    end
...
player.behaviour[player.weapon_states.shooting] = function (self, dt)
    ...
    CreateProjectile(self.x + self.w/2, self.y + self.h/2, 8, 8, dir, 200, self)
